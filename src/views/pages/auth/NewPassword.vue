...
<template>
    ...
    <div class="px-5 min-h-screen flex justify-content-center align-items-center">
        <div class="border-1 surface-border surface-card border-round py-7 px-4 md:px-7 z-1">
            <div class="mb-4">
                <div class="text-900 text-xl font-bold mb-2">Nueva contraseña</div>
                <span class="text-600 font-medium">Ingreso tu nueva clave</span>
            </div>
            <div class="flex flex-column">
                <span class="p-input-icon-left w-full mb-4">
                    <i class="pi pi-lock z-2"></i>
                    <Password id="password1" placeholder="Nueva clave" v-model="value1" class="w-full" :inputStyle="{ paddingLeft: '2.5rem' }" inputClass="w-full md:w-25rem" toggleMask></Password>
                </span>

                <span class="p-input-icon-left w-full mb-4">
                    <i class="pi pi-lock z-2"></i>
                    <Password id="password2" placeholder="Repite tu clave" v-model="value2" class="w-full" :inputStyle="{ paddingLeft: '2.5rem' }" inputClass="w-full md:w-25rem" toggleMask :feedback="false"></Password>
                </span>
                <div class="flex flex-wrap gap-2 justify-content-between">
                    <Button label="Cancelar" class="flex-auto" outlined @click="navigateToDashboard"></Button>
                    <Button label="Guardar cambios" class="flex-auto" @click="navigateToDashboard"></Button>
                </div>
            </div>
        </div>
    </div>
